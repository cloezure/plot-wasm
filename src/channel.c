...
Channel *Channel_service_init(SDL_Point position, int32_t channel_number,
  ...
  Channel_service *new_channel = malloc(sizeof(*new_channel));
  ...
  new_channel->channel_number = Text_init(
      font_type(FONT_B), 150, COLOR_CHANNEL_NUMBER, pos_num, channel_number_s);
  ...
  SDL_Point pos_plot1_body = {.x = pos_plot0_body.x +
                                   new_channel->channel->plot0->position.w + 73,
                              .y = pos_num.y - 25};
  ...
  new_channel->channel->plot0_name =
      Text_init(font_type(FONT_B), plot_size_name, COLOR_PLOT_NAME,
                pos_plot0_name, plot0_name);

  SDL_Rect pos_plot1_name = {.x = pos_plot0_name.x + 280,
                             .y = pos_plot0_name.y};
  new_channel->channel->plot1_name =
      Text_init(font_type(FONT_B), plot_size_name, COLOR_PLOT_NAME,
...
void Channel_service_free(Channel *channel) {
  ...
  Channel_service *schannel = (Channel_service *)channel;
...
Channel *Channel_relay_init(SDL_Point position, int32_t channel_number,
                            char const *plot0_name, char const *plot1_name) {
  Channel_relay *new_channel = malloc(sizeof(*new_channel));
  new_channel->channel = malloc(sizeof(*new_channel->channel));
  new_channel->channel->position = position;
...
void Channel_relay_free(Channel *channel) {
  ...
  Channel_relay *rchannel = (Channel_relay *)channel;
