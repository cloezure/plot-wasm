...
struct chart_points *chart_points_init(char16_t const *unit, int32_t *charts,
  ...
  chpo->length = length;
  chpo->charts_nums = malloc(sizeof(int32_t) * length);
  for (size_t i = 0; i < length; ++i) {
    chpo->charts_nums[i] = charts[i];
  }

  chpo->points = malloc(sizeof(struct text8 *) * length);
  bool sdvig_1 = true;
  bool sdvig_2 = true;
  bool sdvig_3 = true;
  for (size_t i = 0; i < length; ++i) {
  ...
  SDL_Rect mid_poings = chpo->points[length / 2]->position;
  ...
  if (charts_mod == CHARTS_MODE_H) {
    ...
    unit_pos = (SDL_Point){
        .x = mid_poings.x + 10,
        .y = mid_poings.y - ((length + 1) * chpo->points[0]->position.h) - 5};
  }
...
void chart_points_free(struct chart_points *chpo) {
  for (size_t i = 0; i < chpo->length; ++i) {
