...
bool Plot_init(void) {
  ...
  SDL_CreateWindowAndRenderer(WIN_WIDTH, WIN_HEIGHT, 0, &g_win, &g_ren);
...
bool Plot_load(void) {
  ...
  g_coords_info =
      CoordsInfo_init("res/Gilroy-Regular.ttf", 20, color, position);
...
#endif
void set_fps(int32_t fps) { FPS_PLOT = fps; }
...
/*         char buffer[100] = {0}; */
/*         spec_t * spec = Spectrum_init(strlen(slice(view, buffer, offset + 8,
 * offset + spectrum_config_size))); */
...
/*     for(float i = g_trace->x0; i < g_trace->fft_reports_size; i +=
 * g_trace->dx) { */
